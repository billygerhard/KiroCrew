...
async function main() {
  ...
  for (const theme of ['dark', 'light']) {
    ...
    const prChip = page.locator(`a[title="Open ${PR_URL}"]`)
  ...
  await load('dark')
  const kbChip = page.locator(`a[title="Open ${PR_URL}"]`)
  ...
  if (focused !== `Open ${PR_URL}`) throw new Error(`chip did not take focus (activeElement title: ${focused})`)
