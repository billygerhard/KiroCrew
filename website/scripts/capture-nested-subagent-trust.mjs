...
const CONTESTED_PURPOSE = `${CONTESTED_STATE} (its task was continued from a chat that did not start it). Start this task again from a single chat, or approve this request.`
...
async function main() {
  ...
  const observed = {
    ...
    scene3: {
      ...
      toolReviewAttributesTheRun: toolReviewText.includes('The run says:') && toolReviewQuotes > 0,
  ...
  const ok = scene1.running === '1' && scene1.awaiting === '1' && scene1.parkedRow
    ...
    && toolBannerShown && observed.scene3.toolLabeled && observed.scene3.toolCardSaysWhy && toolReviewShown && observed.scene3.toolReviewCarriesRemedy && observed.scene3.toolReviewAttributesTheRun && lightShown
