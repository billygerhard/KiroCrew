...
const CONTESTED_STATE = 'This run was continued from more than one chat'
const CONTESTED_TITLE = 'spawn_run(grandchild task)'
const CONTESTED_PURPOSE = 'Approve it, or start it again from one chat — this run was continued from more than one chat, '
  + 'so no single chat\'s trust applies.'
...
async function main() {
  ...
  await send('approval', {
    ...
    tool_purpose: `${CONTESTED_STATE} (no single chat\'s trust applies). Dry-run the pinned schema migration against staging before applying it`,
    source: 'subagent', ts: Date.now() / 1000,
  }, 1200)
  ...
  if (toolBannerShown) {
    ...
    if (toolReviewShown) {
      await panelTitle.locator('xpath=ancestor::div[contains(@class,"h-full")][1]').screenshot({ path: `${OUT}/3c-contested-tool-prompt-review-panel.png` })
    }
  }

  // ── Scene 3e: the contested run's CARD, in the continuing chat's tab ───────
  // The prompt above reached only the feed, but the run's card lives in the
  // tab of the chat that continued it. `_interactive_approval` brackets the
  // parked wait with `subagent_awaiting_feed` frames to that tab, so the chip
  // row says where the prompt went instead of reading as a stall.
  await page.keyboard.press('Escape')
  await page.waitForTimeout(400)
  await page.keyboard.press('Escape')
  await page.waitForTimeout(600)
  await send('subagent_spawn', { slot: SLOT, id: CONTESTED_ID, task: 'continue the migration review from this chat', agent: 'kirocrew' })
  await send('subagent_tool', { slot: SLOT, id: CONTESTED_ID, tool: 'shell', tool_count: 2 })
  await send('subagent_awaiting_feed', { slot: SLOT, id: CONTESTED_ID, awaiting: true }, 600)
  const feedHint = page.getByTestId('subagent-awaiting-feed').first()
  const feedHintShown = await feedHint.count() > 0
  const feedHintText = feedHintShown ? (await feedHint.textContent() || '') : ''
  await shotChip('3e-contested-card-row-bell-hint.png')
  await send('subagent_awaiting_feed', { slot: SLOT, id: CONTESTED_ID, awaiting: false }, 600)
  const feedHintCleared = (await page.getByTestId('subagent-awaiting-feed').count()) === 0
  ...
  const observed = {
    ...
    scene3: {
      ...
      explained: bannerText.includes('Approve it, or start it again from one chat'),
      reviewShown,
      reviewWhole: reviewText.includes('so no single chat\'s trust applies') && reviewText.includes('spawn_run(grandchild task)'),
      lightShown,
      toolBannerShown,
      toolLabeled: toolBannerText.includes('shell(psql'),
      toolReviewShown,
      feedHintShown,
      feedHintNamesBell: feedHintText.includes('notification bell'),
      feedHintCleared,
    },
  }
  ...
  const ok = scene1.running === '1' && scene1.awaiting === '1' && scene1.parkedRow
    ...
    && reviewShown && observed.scene3.reviewWhole
    && toolBannerShown && observed.scene3.toolLabeled && toolReviewShown && lightShown
    && feedHintShown && observed.scene3.feedHintNamesBell && feedHintCleared
