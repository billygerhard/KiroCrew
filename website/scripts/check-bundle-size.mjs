...
export const CHUNK_BUDGETS = {
  ...
  // The per-chat backend surface (#11863) -- the composer chip, the Welcome-view
  // pick wiring and the slot field it reads -- adds 4,581 B of first-party
  // app-core code on top of main (the picker's own dropdown, `BackendSelector`,
  // is behind a lazy boundary in WelcomeView and is not in this chunk). Its merge
  // result builds inside the ceiling above, so the number does not move for it;
  // the cost is recorded here so the next re-measure can attribute the chunk's
  // growth.
