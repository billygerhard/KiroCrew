 ...
 *
 * Twelve frames, one per user-visible surface the change adds, plus a frame
...
const BACKENDS = {
  ...
  unroutable: [
    ...
    { id: 'orbit', label: 'Orbit Agent', reason: 'routing not yet verified end to end: run Verify in Settings → AI backends (the gateway spawns the harness once and checks that a tool call asks for permission before acting)', verifiable: true },
  ],
}
...
const extra = async (path, route) => {
  ...
  if (path === '/api/backends/orbit/verify') {
    ...
    if (answer.verified) verifyDone = true
  ...
  if (path === '/api/backends') {
    ...
    return await json(route, verifyDone ? VERIFIED_BACKENDS : BACKENDS), true
  }
...
async function captureInheritedChip(browser, base) {
  // Frame 5: the UNPINNED chat's chip. It follows the configured default, so
  // it carries no lock glyph and its click-to-explain caption says so -- the
  // counterpart to frame 4's pinned explanation. The welcome screen's shelf is
  // where an unpinned chat's chip first shows.
  const { context, page } = await newPage(browser, EMPTY_SLOT)
  await page.goto(base + '/chat', { waitUntil: 'domcontentloaded' })
