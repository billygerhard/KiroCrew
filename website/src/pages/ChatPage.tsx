 ...
 // forensic attribution. Set on widget pre-fill, consumed
  ...
  const send = useCallback(async (optionText?: string, targetSlot?: string, steerNow?: boolean, isolated = false): Promise<boolean> => {
    ...
    if (!slot || forceNew) {
      ...
      try {
        created = await dispatch(createSlot({ agent: pendingAgentRef.current || defaultAgent || undefined, agent_kind: pendingAgentRef.current ? pendingAgentKindRef.current : undefined, model: pendingModelRef.current || undefined, backend: pendingBackendRef.current || undefined, mode: modeRef.current })).unwrap()
      } catch (e: unknown) {
        ...
        {splitMode && splitFeatureEnabled ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
            ...
            <Btn
              ...
              onClick={() => {
                ...
                dispatch(createSlot({ agent: pendingAgent || defaultAgent || undefined, agent_kind: pendingAgent ? pendingAgentKindRef.current : undefined, model: pendingModel || undefined, backend: pendingBackend || undefined, mode }))
              }}
            >
          ...
          <div className="relative flex flex-col flex-1 min-h-0" {...dropTargetProps}>
            ...
            <div className="relative">
              ...
              backendLabel={backendChoiceExists ? backendChipLabel : undefined}
              backendTitle={backendChipTitle}
              backendIsInheritedDefault={!activeSlotBackend}
