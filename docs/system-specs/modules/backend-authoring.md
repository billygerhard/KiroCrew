...
A claim takes effect at registration: each `true` flag is recorded against the
descriptor's id (`register_known_backend(capabilities=...)`) and the session path
reads membership through the derived `<capability>_backends()` accessors in
`agent_sdk.backends` — the frozen `ACP_BACKENDS_*` vocabulary plus the registered
claims — at every gate that used to read the frozen set alone. So a descriptor
that claims `model_via_config_option` takes the config-option model-switch branch
a builtin member takes, while the vocabulary set itself stays a literal (a
builtin's membership remains a visible edit). A `false` or absent flag records
nothing and the host keeps the default posture.
