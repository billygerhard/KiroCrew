...
Operator descriptors live under `agent.harnesses` in `config.json` (schema in
[config.md](config.md)), keyed by harness id:
...
`agent.harnesses` is in the config **write-guard**: a descriptor names a binary
Kiro Crew spawns, so an agent-invoked tool cannot create or modify one — only an
operator editing `config.json` (or `config.local.json`) can. Edit the file, then
the descriptor is picked up on the next config load; `GET /api/harnesses` and
**Settings → AI Backend** list it alongside the bundled rows. A descriptor that
fails validation is not silently dropped: it is served separately under the
listing's `invalid` array with a field-named reason, and the Settings row shows
that reason inline (R6.1).

To make the harness the default for unselected sessions, set
`agent.default_harness` to its id; to bind a single chat, pick it in the chat
composer's harness picker (see [providers.md](providers.md) § Chat composer).
...
- **`agy`** (the key) is the harness id. Lowercase letters, digits, and hyphens
  only, ≤ 64 characters, unique across bundled and operator harnesses. It is the
...
| Field | Required | Rule |
|---|---|---|
| `id` (map key) | yes | Non-empty; lowercase letters/digits/hyphens; ≤ 64 chars; unique across all harnesses. |
