...
4. A new harness that ships IN THIS TREE adds rows to `ACP_BACKENDS_KNOWN`, a
   ...
   routing and opt-in capabilities are recorded by `register_known_backend` at
   boot (`docs/system-specs/modules/backend-authoring.md`); a capability it does
   not claim is a Group B decision of "not a member", and the loader refuses
