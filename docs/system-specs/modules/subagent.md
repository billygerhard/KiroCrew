...
- `reasoning_effort` (str, optional): per-call reasoning-effort override (`low`/`medium`/`high`/`xhigh`/`max`), batch-wide like `model`. Precedence: per-call value → `agent.role_efforts['subagent']` pin → provider default; `""`/absent changes nothing. Like a model/effort role pin, a non-empty value forces the dedicated-process path (the parent's shared runtime cannot switch effort per session), so a wide fan-out pays a full process per subagent — and that cost is paid even when the resolved model turns out not to support effort (the level is then dropped at the provider factory). Carried through the stagger queue and the retry endpoint like the context-group flags. NOT inherited by `spawn_continue` — a continuation resolves effort fresh (role pin, else default), the same parity as `model`. When the requested effort cannot take effect, the gateway says so: `/api/spawn` resolves the model the factory's effort gate will see (per-call value, else the subagent role pin, else the session chain for the effective agent — a crew's own model pin, else a non-sentinel global `agent.model`; a named kiro agent's own pin resolves downstream and cannot carry the overlay) and returns an `effort_dropped` reason on the success response, which the tool renders as one attributed line per distinct verdict — subagents sharing an identical verdict (the usual case, since the value is batch-wide) are collapsed into a single line naming all of them, while differing verdicts keep their own attributed lines — including the default case where nothing is pinned and the model resolves to "auto". When the effort WILL apply, the response instead carries an `effort_applied` note naming the resolved model and the family-specific settings key (`reasoning` for GPT, `output_config` for Claude) it is delivered under, rendered the same way — so both outcomes of a requested effort are visible in the tool result. A role-pinned effort that will be dropped (no per-call effort involved) still surfaces in the gateway log at warning level, since the tool caller never asked for it — that warning is emitted by the provider factory's effort gate itself (`config/loader.py`), the single authority that drops the level, so one log line covers every surface that funnels through it (spawn, dashboard slot, cron) and cannot drift from the decision it reports on. The HARNESS half of the same question travels on the same key: when the resolved harness's capability set lacks `reasoning_effort`, `effort_dropped` carries a reason naming that harness instead, and it is checked FIRST — a harness that cannot honour effort at all makes the model's capability moot, and reporting the model reason there would send the caller to change a pin that would not help. The provider factory remains the single dropping authority for the model half; the report never rejects or alters a spawn. Per-TASK variation inside one call is deliberately not supported (see issue #2140).
- `harness` (str, optional): ACP harness id for the spawn, batch-wide like `model`. Resolution order: the explicit value → the spawning session's own bound harness → the configured default (`agent.default_harness`, else the `agent.acp_backend` alias). Refusal over fallback: an unknown, unavailable, or not-yet-serviceable id fails THAT spawn with a reason naming the harness, and its batch siblings are unaffected — a spawn is never quietly served by a harness the caller did not name. An explicit value and an inherited binding are both availability-checked before dispatch — the inherited one WITHOUT the recorded-spawn-failure question, since the session it comes from is alive on that harness — and only the truly-defaulted path (no explicit value and no binding to inherit) is un-probed: it performs no executable probe at all, so a spawn that selects nothing behaves exactly as it did before this parameter existed, and kiro-cli's absence still surfaces from the spawn itself. A model pinned alongside is validated against the selected harness's model source, but only when that harness DECLARES its catalog (`model_source: static`): a harness that enumerates over ACP has no catalog before a session exists, so a pin on it is carried through and answered by the harness. A `reasoning_effort` level is DROPPED with a report when the resolved harness's capability set lacks `reasoning_effort` — dropped rather than passed through, because a level the harness cannot honour would still force the dedicated-process path for no effect. The report shares the `effort_dropped` key with the model-capability verdict described above rather than adding a second channel, so a caller reads one answer to "is my effort in effect". The RESOLVED id is what the run records, including the inherited and defaulted cases, and it is what the queue round-trip carries (a drained member is not re-validated: its harness was judged at submission). It is also THREADED into session creation, not merely recorded: an explicit pick and an inherited binding reach `get_or_create` as that id, marked pre-bound so the second resolution asks the same question the pre-dispatch gate did rather than refusing for a failure recorded in between; a defaulted run and a continuation pass `""`, the first so the un-probed default path stays un-probed and the second so session creation resumes the conversation's own recorded binding. Recording and reporting a harness the session was never created with is the same substitution this parameter exists to prevent, one layer down: an empty value binds the configured DEFAULT on a fresh session, so a spawn from a KAS-bound parent would run on kiro-cli while every surface said KAS. A subagent bound to a different harness than its parent takes the dedicated-process path, since the shared runtime is one harness's process. Not inherited by `spawn_continue`: a continuation resolves and attributes from the CONVERSATION's own recorded binding — seeded from the run's `state.json` when the session map no longer holds one — never from the session dispatching the follow-up, so it is served by the harness holding the transcript and is not refused because the dispatcher's harness is unavailable.
...
- Attribution travels with every accepted run: the spawn response echoes the resolved `harness` (and `effort_dropped` when a level was dropped for it), the `subagent_spawn` / `subagent_done` events and the completion-card meta carry the harness beside the served model, and `spawn_list` renders `harness: <id> (<served model>)`. A record with no harness renders as unknown rather than as the default harness, because naming a harness a run may not have used is worse than naming none.
