...
**Key-level config protection** (`security.protected_config_write` + `_WRITE_PROTECTED_CONFIG_KEYS`, consulted by `cli_config.py`) — the two guards above reason about a *file*, which is the right shape for values whose worst case the load-time clamp neutralizes. A few keys break that assumption and need a guard at the *key*, because `kirocrew config set` is an ordinary command an agent runs through its bash tool: it takes the config lock itself so it never reaches the file-edit gate, and `--local` writes the overlay, which takes *precedence* over the base file.
- **`agent.harnesses`** is the first and currently only entry, and it states the bar: its value names an **executable the gateway spawns** plus the argv it spawns it with, and *nothing downstream clamps that* — the harness registry validates a descriptor's shape, not its intent, so a shape-valid descriptor pointing at an attacker-written script is honoured exactly as an operator's would be and is exec'd in the gateway's own identity.
- `protected_config_write(key, value, existing)` returns a *reason* (empty when allowed) and matches on the key **path**, not on a string prefix, covering three forms because all three write the same value: the key itself, any path inside it (`agent.harnesses.mine.executable`), and an ancestor write that **replaces the node holding it** (`config set agent '{…}'`, and the whole-object `config set --file`, which passes `key=""`). The ancestor form is refused on either of two grounds, so a section write that neither authors nor destroys a definition still works: the incoming **value** carries the protected key (the write authors one), or the **on-disk** value carries it (the write deletes one — setting a parent replaces the whole node beneath it, so `config set agent '{"approval_mode": "auto"}'` wipes an operator's harnesses while naming none, and deletion is a modification). The delete half needs the file's current contents, so `cli_config.py` passes the raw file the write lands in — the base config, or the overlay for `--local`, never the merged config, since the two merge per leaf and an overlay write cannot destroy a base key. That read happens before the config lock, so a definition added inside that window is not seen. Refusals are SEL-audited with `outcome="denied"`.
- **Residual, accepted deliberately:** neither raw-shell form — `echo '{…}' > ~/.kiro/crew/config.json` nor its `cd`-relative equivalent — is refused, because closing them means adding `config.json` to `_WRITE_PROTECTED_BASH_LEAVES`, whose matcher is verb-independent and would therefore deny the routine config *reads* the write-only tier exists to keep allowed. The cost is bounded by the registry never trusting on-disk content it cannot validate (an invalid descriptor is excluded from every listing with a recorded reason), so what remains is a **shape-valid** malicious descriptor planted by raw redirect — real, and the same exposure `config.json` carries generally today. This guard closes the setter, not the file.
