...
- **App approval-grant trust root (keystone leaf `app_approval_grants.json`)** — `~/.kiro/crew/app_approval_grants.json` maps an app name to the tool-approval posture the operator granted it (`apps/approval_grants.py`), and is added to `_CREW_SECRET_LEAVES` so it is read+write-blocked under every `_CREW_HOME_PREFIXES` entry on both the tool path and every shell form. An app's manifest (`permissions.approvalMode`) only DECLARES a wanted posture; this file is what makes it apply, so an agent that could write it could grant its own app auto-approval for every tool call in the sessions that app seeds, and one that could read it would learn which apps already run unattended. The loader (`load_app_approval_grants`) opens it directly rather than through the gate; the operator edits it out-of-band. Both failure modes withhold: an absent file grants nothing, and a present-but-unreadable file grants nothing and emits a critical SEL event.
