...
def found_exe(monkeypatch):
    ...
    monkeypatch.setattr(rv, "spawn_attestation_problem", lambda descriptor, exe: None)
...
async def test_spawn_refuses_when_the_verified_binary_changed_and_revokes(
    ...
    try:
        ...
        rv.record_attestation(d, mechanism="agent_spec", evidence={})
        ...
        # Unchanged binary: the spawn goes through.
        plan = await harness.resolve_spawn(_ctx(tmp_path))
