...
def test_a_descriptors_capability_claims_reach_the_readers(clean_boot, tmp_path):
    """The ``capabilities`` block is a live claim, not a stored annotation.

    Each True capability grows the ``<capability>_backends()`` accessor the
    client/session-handle read at every gate site, so a config-authored host that
    claims ``model_via_config_option`` takes the config-option model-switch path a
    builtin member takes. Unclaimed capabilities carry the frozen set's default
    posture, and the frozen vocabulary itself is never widened.
    """
    exe = _stub_executable(tmp_path)
    path = _write_harnesses(
        tmp_path,
    ...
    # Claimed -> member through the accessor the readers use.
    assert "my-acp" in b.model_via_config_option_backends()
    assert "my-acp" in b.advertised_model_selection_backends()
    # Unclaimed (False or absent) -> default posture.
    assert "my-acp" not in b.load_without_modes_backends()
    assert "my-acp" not in b.session_mcp_array_backends()
    assert "my-acp" not in b.harness_owned_sessions_backends()
    # The vocabulary is a literal and stays one.
    assert "my-acp" not in b.ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION
    assert "my-acp" not in b.ACP_BACKENDS_ADVERTISED_MODEL_SELECTION

    # And the SDK's per-session capability view reflects the claim.
    from kiro_crew.agent_sdk.capabilities import capabilities_for

    assert capabilities_for("my-acp").resolves_model_from_advertised_list is True


def test_every_grantable_capability_is_read_somewhere_on_the_session_path():
    """No capability may be accepted by the schema and consumed by nobody.

    Pins the defect this seam closes: five names were validated and stored while
    every gate read the frozen set directly, so a claim was inert. Each accessor
    must be referenced by at least one module outside the vocabulary home.
    """
    import inspect

    from kiro_crew.acp import client as client_mod
    from kiro_crew.acp import session_handle as handle_mod
    from kiro_crew.agent_sdk import capabilities as caps_mod

    readers = "\n".join(inspect.getsource(m) for m in (client_mod, handle_mod, caps_mod))
    for name, accessor in b.CAPABILITY_BACKENDS_ACCESSORS.items():
        assert accessor.__name__ + "()" in readers, f"{name} has no reader on the session path"


def test_no_session_path_module_reads_a_grantable_capability_off_the_frozen_set():
    """A grantable claim must be honoured on EVERY path a session can take.

    The reader test above proves each accessor has at least one consumer; this
    one proves no consumer bypasses it. ``AcpProvider._start_kiro_runtime_impl``
    (the ``AcpRuntime`` resume path, the one an operator runtime backend takes)
    read the frozen ``ACP_BACKENDS_HARNESS_OWNED_SESSIONS`` directly, so a
    descriptor claiming ``harness_owned_sessions`` passed the accessor test yet
    still got the kiro-transcript pre-check there and never resumed. Membership
    tests against a grantable name's frozen set are forbidden outside the
    vocabulary module; docstrings and comments may still name the set.
    """
    import ast
    import inspect
    ...
    frozen = {"ACP_BACKENDS_" + name.upper() for name in b.CAPABILITY_BACKENDS_ACCESSORS}
    offenders: list[str] = []
    for mod in (client_mod, handle_mod, caps_mod, provider_mod):
        tree = ast.parse(inspect.getsource(mod))
        for node in ast.walk(tree):
            if isinstance(node, ast.Name) and node.id in frozen and isinstance(node.ctx, ast.Load):
                offenders.append(f"{mod.__name__}:{node.lineno} {node.id}")
    assert offenders == [], "grantable capability read off the frozen set: " + ", ".join(offenders)
