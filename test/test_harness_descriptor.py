...
def test_argv_reasons_name_the_token_by_position_never_by_content():
    ...
    assert any("argv[2] uses unknown placeholder {nope}" in r for r in reasons)
...
def test_loader_too_many_descriptors_is_a_whole_file_invalid_entry(tmp_path):
    ...
    p = tmp_path / "harnesses.json"
    too_many = {
        f"h{i}": {"executable": "x", "argv": ["{executable}"], "routing": "agent_spec"}
...
def test_loader_duplicate_key_refuses_the_file_instead_of_keeping_the_last(tmp_path):
    ...
    whole-file refusal with the key named."""
    p = tmp_path / "harnesses.json"
    first = '"dup": {"executable": "/opt/a", "argv": ["{executable}"], "routing": "agent_spec"}'
    second = '"dup": {"executable": "/opt/b", "argv": ["{executable}"], "routing": "agent_spec"}'
    p.write_text("{" + first + ", " + second + "}", encoding="utf-8")
    valid, invalid = load_operator_descriptors(path=p)
    assert valid == ()
    assert invalid[0][0] == ""
    assert any("duplicate key 'dup'" in r for r in invalid[0][1]), invalid

    # Nested: a descriptor that names ``executable`` twice is refused the same way.
    p.write_text(
        '{"one": {"executable": "/opt/a", "executable": "/opt/b", '
        '"argv": ["{executable}"], "routing": "agent_spec"}}',
        encoding="utf-8",
    )
    valid, invalid = load_operator_descriptors(path=p)
    assert valid == ()
    assert any("duplicate key 'executable'" in r for r in invalid[0][1]), invalid
