...
class TestNestedTrustInheritance:
    ...
    def test_a_pre_stamp_depth_one_founder_derives_its_root_from_its_chat_parent(self) -> None:
        ...
        assert nested.conversation_root_session_key == CONTESTED_A
    ...
    async def test_cancelled_requester_cannot_inherit_the_continuing_chats_trust(self) -> None:
        ...
        assert purpose.startswith(
            "Approve it, or start it again from one chat — this run was continued"
        )
        assert purpose.endswith("from more than one chat, so no single chat's trust applies.")
...
class TestNestedPromptRoutesToRootSlot:
    ...
    async def test_a_contested_runs_tool_prompt_says_why_it_has_no_chat(self) -> None:
        ...
        assert (
            contested.tool_purpose
            == f"{CONTESTED_PROMPT_STATE} ({CONTESTED_PROMPT_WHY}). Dry-run the pinned migration"
        )
        # The spawn prompt's state, verbatim: what the user did, in their words.
        assert CONTESTED_PROMPT_STATE == "This run was continued from more than one chat"
        assert (
            CONTESTED_PROMPT_REMEDY not in contested.tool_purpose
        )  # the remedy stays on the spawn prompt
    ...
    def test_a_contested_prompts_card_tab_resolves_from_its_run(self) -> None:
        """The tab that shows a contested run's card, so it can be told where the prompt went.

        A spawn prompt names its run in the request id; a tool prompt carries the
        marker, whose conversation key names the founder, and the one live run of
        that conversation is the asker. An owned prompt, an unresolvable run, or a
        run with no tab of its own (its routing root is a marker) answers nothing.
        """
        manager = _manager(_sessions({ROOT}), AsyncMock(return_value=True))
        original = _live(manager, "A", "dashboard:chat-2")
        original.done = True
        a2 = _continue(manager, "A2", ROOT, "subagent:A")  # card in chat-1, prompt contested
        gateway = _gateway(manager)
        resolve = gateway._contested_prompt_card  # type: ignore[attr-defined]

        assert resolve("spawn:A2", CONTESTED_A) == ("chat-1", "A2")
        assert resolve("req-7", CONTESTED_A) == ("chat-1", "A2")  # tool prompt: the live run
        assert resolve("req-7", ROOT) == ("", "")  # owned prompt: already in its tab
        assert resolve("spawn:nope", CONTESTED_A) == ("", "")
        b = _live(manager, "B", "subagent:A2")  # a child: its own root IS the marker, no tab
        assert b.root_session_key == CONTESTED_A
        assert resolve("spawn:B", CONTESTED_A) == ("", "")
        a2.done = True  # no live run of the conversation: a tool prompt resolves nothing
        assert resolve("req-8", CONTESTED_A) == ("", "")

    @pytest.mark.asyncio
    async def test_the_cards_tab_is_told_while_a_contested_prompt_is_parked(self) -> None:
        """``subagent_awaiting_feed`` brackets exactly the parked wait on the card's tab.

        The prompt itself is unowned (``slot=""``); the frames go to the tab the
        card lives in, set before the prompt is raised and cleared however it ends.
        """
        from kiro_crew.llm_helpers import LLMEvent
        ...
        gateway.dashboard_state.broadcast_ws = MagicMock(  # type: ignore[attr-defined]
            side_effect=lambda t, d: frames.append((t, d))
        )
        gateway.dashboard_state._dashboard_client_attached = MagicMock(return_value=True)  # type: ignore[attr-defined]

        async def _request(*_a: object, **kw: object) -> bool:
            frames.append(("request_approval", dict(kw)))
            return False

        gateway.dashboard_state.request_approval = AsyncMock(side_effect=_request)  # type: ignore[attr-defined]
        with (
            patch("kiro_crew.slack.gateway.safety_override") as override,
            patch("kiro_crew.slack.gateway.sel"),
        ):
            ...
            approve = gateway._interactive_approval("subagent")  # type: ignore[attr-defined]
            event = LLMEvent(kind="permission_request", request_id="req-1", title="shell: ls")
            approved = await approve(event, CONTESTED_A)
        ...
        assert frames[0][1] == {"slot": "chat-1", "id": "A2", "awaiting": True}
        assert frames[1][1]["slot"] == ""  # the prompt itself stays unowned
        assert frames[2][1] == {"slot": "chat-1", "id": "A2", "awaiting": False}
...
class TestRecoveredRowsFailClosed:
    ...
    async def test_recovered_row_is_interactive_even_when_a_re_walk_finds_a_trusted_chat(
        ...
        assert (
            "\nApprove it, or start it again from one chat — this run was continued" in description
        )

    @pytest.mark.asyncio
    async def test_a_resumed_admission_without_its_stamp_fails_closed_in_the_gate(self) -> None:
        ...
        re-entry) share one branch and mark a resumed admission. Without the stamp a nested caller
        resolves to a contested root inside the gate, whatever the walk would
        now find; with the stamp, and for a parentless run, nothing changes.
        """
        ...
        async def _resumed(**kw: object) -> SubagentInfo:
            ...
            with patch("kiro_crew.subagent.Stats"), patch("kiro_crew.subagent.sel"):
                info = manager.spawn("t", _from_queue=True, **kw)  # type: ignore[arg-type]
                assert info is not None and not info.queued
                await manager._tasks[info.id]
            return info
