...
class TestNestedTrustInheritance:
    ...
    async def test_cancelled_requester_cannot_inherit_the_continuing_chats_trust(self) -> None:
        ...
        # State first (the card's two lines say which verdict this is), the why
        # in a clause, then the remedy -- the same shape as a tool prompt's note.
        assert purpose == (
            "This run has no single owning chat (its task was continued from a chat that did not start it)."
            " Start this task again from a single chat, or approve this request."
        )
...
class TestNestedPromptRoutesToRootSlot:
    ...
    async def test_a_contested_runs_tool_prompt_says_why_it_has_no_chat(self) -> None:
        ...
        # State and why first (the card's two lines), then what the tool is
        # for, then the remedy LAST -- whole in the Review panel, so a user who
        # only ever meets tool prompts still finds the way out where they decide.
        # Three paragraphs: the system's state, the RUN's own claim about the
        # tool -- led with who is speaking and set as a quote, so on the one
        # surface that exists because the run is not trusted its claim never
        # reads with the system's authority -- then the system's remedy.
        assert contested.tool_purpose == (
            f"{CONTESTED_PROMPT_STATE} ({CONTESTED_PROMPT_WHY}).\n\n"
            f"{CONTESTED_PROMPT_RUN_SAYS}\n\n> Dry-run the pinned migration."
            f"\n\n{CONTESTED_PROMPT_REMEDY}."
        )
...
class TestRecoveredRowsFailClosed:
    ...
    async def test_recovered_row_is_interactive_even_when_a_re_walk_finds_a_trusted_chat(
        ...
        assert (
            "\nThis run has no single owning chat (its task was continued from a chat that did not start it)."
            " Start this task again from a single chat, or approve this request." in description
        )
