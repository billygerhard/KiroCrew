...
class TestSignalConstants:
    ...
    def test_extra_signals_widen_the_manifest_set(self) -> None:
        manifests = recognized_manifests(["BUILD.bazel"])

        assert "BUILD.bazel" in manifests
        assert frozenset(MANIFESTS) <= manifests

    def test_extra_signals_cannot_shrink_the_manifest_set(self) -> None:
        assert recognized_manifests(()) == frozenset(MANIFESTS)

    def test_blank_extra_signals_are_dropped(self) -> None:
        # An empty or whitespace name matches no file but reads as configured;
        # dropping it keeps "configured" and "effective" the same thing.
        assert recognized_manifests(["", "   ", "\t"]) == frozenset(MANIFESTS)

    def test_extra_signals_are_stripped(self) -> None:
        assert "BUILD.bazel" in recognized_manifests([" BUILD.bazel "])
