...
class TestSlotHistoryKey:
    ...
    def test_a_dashboard_slot_merely_NAMED_like_a_channel_keeps_its_own_file(
        ...
        """A filename shape is not provenance.

        ``POST /api/chat/slots`` accepts a client-supplied name, so a fresh
        dashboard conversation can be created under a `slack_<ts>` name that
        collides with a real thread's transcript. It must keep its own
        ``dashboard:`` transcript rather than appending into that thread and
    ...
    def test_restore_marks_a_channel_tab_as_channel_origin(self, tmp_path):
        """The restore adopts an existing transcript, so it sets provenance."""
        state = _state(tmp_path)
