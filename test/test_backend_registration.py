...
# ── Config-grantable capabilities ──


class TestRegisteredCapabilities:
    """A descriptor's capability claims grow the derived accessors, never the vocabulary."""

    def test_accessors_equal_their_frozen_sets_at_rest(self, clean_registry):
        for name, accessor in b.CAPABILITY_BACKENDS_ACCESSORS.items():
            frozen = getattr(b, "ACP_BACKENDS_" + name.upper())
            assert accessor() == frozen, name

    def test_every_grantable_capability_has_an_accessor(self):
        assert set(b.CAPABILITY_BACKENDS_ACCESSORS) == set(b.CONFIG_GRANTABLE_CAPABILITIES)

    def test_a_claimed_capability_grows_only_its_accessor(self, clean_registry):
        b.register_known_backend(
            "zz-cap",
            label="Z",
            routing=b.Routing.AGENT_SPEC,
            capabilities={
                b.CAPABILITY_MODEL_VIA_CONFIG_OPTION: True,
                b.CAPABILITY_LOAD_WITHOUT_MODES: False,
            },
        )
        assert "zz-cap" in b.model_via_config_option_backends()
        # False and absent claims record nothing: the default posture.
        assert "zz-cap" not in b.load_without_modes_backends()
        assert "zz-cap" not in b.session_mcp_array_backends()
        assert "zz-cap" not in b.harness_owned_sessions_backends()
        assert "zz-cap" not in b.advertised_model_selection_backends()
        # The vocabulary set is a literal and stays one.
        assert "zz-cap" not in b.ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION

    @pytest.mark.parametrize(
        "name", ["session_sharing", "internal_sandbox", "acp_runtime_pool", "telepathy"]
    )
    def test_a_non_grantable_capability_is_refused_before_any_mutation(self, clean_registry, name):
        with pytest.raises(ValueError, match="not config-grantable"):
            b.register_known_backend(
                "zz-bad", label="Z", routing=b.Routing.AGENT_SPEC, capabilities={name: True}
            )
        assert "zz-bad" not in b.known_backends()
        assert all("zz-bad" not in acc() for acc in b.CAPABILITY_BACKENDS_ACCESSORS.values())

    def test_the_reset_clears_claims(self, clean_registry):
        b.register_known_backend(
            "zz-cap",
            label="Z",
            routing=b.Routing.AGENT_SPEC,
            capabilities={b.CAPABILITY_SESSION_MCP_ARRAY: True},
        )
        assert "zz-cap" in b.session_mcp_array_backends()
        b._reset_registered_backends()
        assert b.session_mcp_array_backends() == b.ACP_BACKENDS_SESSION_MCP_ARRAY
