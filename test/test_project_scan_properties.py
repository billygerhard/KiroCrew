...
_EXTRA_SIGNALS = st.sampled_from(((), ("BUILD.bazel",), ("BUILD.bazel", "  "), ("notes.txt",)))
...
def _manifest_names(extra_signals: tuple[str, ...]) -> set[str]:
    """Return the manifest filenames a scan with ``extra_signals`` recognizes."""

    return set(MANIFESTS) | {name.strip() for name in extra_signals if name.strip()}
...
def test_two_scans_of_an_unchanged_tree_agree(
    ...
    first = scan(root, extra_signals=extra_signals, depth_cap=depth_cap)
    second = scan(root, extra_signals=extra_signals, depth_cap=depth_cap)
...
def test_candidates_stay_inside_the_scan_root(
    ...
    tree = scan(root, extra_signals=extra_signals, depth_cap=depth_cap)
...
def test_every_tier_is_justified_by_what_is_on_disk(
    ...
    root, _ = _build_tree(scan_area, spec)
    manifest_names = _manifest_names(extra_signals)
