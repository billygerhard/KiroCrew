...
class TestRedactCredentialsUnit:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # --- scp-style dotless host with colon-path (pass 3 alternation B) ---
            (
                "tok_secret123@forge:team/repo.git",
                "<redacted>@forge:team/repo.git",
            ),
            (
                "tok_secret123@buildbox:",
                "<redacted>@buildbox:",
            ),
            # Dotless host in ssh permission-denied context
            (
                "tok_secret123@forge: Permission denied (publickey).",
                "<redacted>@forge: Permission denied (publickey).",
            ),
            # --- scp-style bracketed IPv6 host (pass 3 alternation C) ---
            (
                "tok_secret123@[::1]:team/repo.git",
                "<redacted>@[::1]:team/repo.git",
            ),
            (
                "tok_secret123@[2001:db8::7]:team/repo.git",
                "<redacted>@[2001:db8::7]:team/repo.git",
            ),
            # Bracketed IPv6 with zone-id
            (
                "tok_secret123@[fe80::1%25eth0]:repo.git",
                "<redacted>@[fe80::1%25eth0]:repo.git",
            ),
            # --- scp-style un-bracketed IPv6 (pass 3 alternation D) ---
            # As echoed by SSH after git strips brackets from SCP remotes.
            (
                "tok_secret123@::1: Permission denied (publickey).",
                "<redacted>@::1: Permission denied (publickey).",
            ),
            (
                "tok_secret123@2001:db8::7: Permission denied",
                "<redacted>@2001:db8::7: Permission denied",
            ),
            # Un-bracketed IPv6 with zone-id (% not encoded in SSH output)
            (
                "tok_secret123@fe80::1%eth0: Connection refused",
                "<redacted>@fe80::1%eth0: Connection refused",
            ),
            # --- non-credential prose (no dot, no colon-path) → NOT redacted ---
            ("see user@word for details", "see user@word for details"),
        ],
        ids=lambda v: v[:50],
    )
...
class TestSchemeAgnosticRedaction:
    """Regression: ftp/ftps/exotic-scheme URLs must have post-authority redacted.

    At 7bb1a016 the _URL_AFTER_AUTHORITY_RE enumerated only https?|ssh|git,
    so FTP(S) and remote-helper schemes (e.g. svn+ssh) leaked path/query
    credentials through all three passes.  The fix makes the scheme match
    generic (RFC 3986 § 3.1) — this class pins that closure.
    """

    def test_ftps_path_token(self):
        """ftps://host/token/repo → token redacted, host preserved."""
        sys.path.insert(
            0,
            str(
                REPO_ROOT
        ...
        stderr_line = (
            ...
            "'ftps://git.example.com/tok_secret123/Repo.git': "
            "Connection refused"
        )
        result = redact_credentials(stderr_line)
        assert "tok_secret123" not in result
        assert result == (
            "fatal: unable to access " "'ftps://git.example.com/<redacted>': " "Connection refused"
        )

    def test_ftp_query_token(self):
        """ftp://host?private_token=secret → query redacted, host preserved."""
        sys.path.insert(
            0,
            str(
                REPO_ROOT
        ...
        stderr_line = (
            "fatal: unable to access "
            "'ftp://git.example.com?private_token=tok_secret123': "
        ...
        assert result == (
            "fatal: unable to access "
            "'ftp://git.example.com/<redacted>': "
            "Could not resolve host"
        )

    def test_exotic_scheme_svn_ssh(self):
        """svn+ssh://host/token/path → token redacted, host preserved.

        svn+ssh is matched by the old code too (ssh:// substring), so this
        test also asserts that the new generic scheme captures the FULL scheme
        prefix (svn+ssh://) rather than just the ssh:// suffix.  Additionally,
        bzr:// is tested as a pure exotic scheme that has no substring overlap
        with the old enumerated set — it pins the class closure.
        """
        sys.path.insert(
            0,
            str(
        ...
        # svn+ssh — exercises the "+" in RFC 3986 scheme grammar
        stderr_line = (
            "fatal: unable to access "
            "'svn+ssh://git.example.com/tok_secret123/x': "
            "Network unreachable"
        )
        result = redact_credentials(stderr_line)
        assert "tok_secret123" not in result
        assert result == (
            "fatal: unable to access "
            "'svn+ssh://git.example.com/<redacted>': "
            "Network unreachable"
        )

        # bzr — pure exotic scheme with zero substring overlap to https?|ssh|git
        bzr_line = (
            "fatal: unable to access "
            "'bzr://git.example.com/tok_secret123/branch': "
            "Connection refused"
        )
        bzr_result = redact_credentials(bzr_line)
        assert "tok_secret123" not in bzr_result
        assert bzr_result == (
            "fatal: unable to access " "'bzr://git.example.com/<redacted>': " "Connection refused"
        )


class TestIPv6UserinfoComposition:
    """Regression: IPv6 URLs with userinfo must have post-authority path redacted.

    At bee1a6b0 (round 12), pass 1 rewrites ``https://user:tok@[::1]/secret``
    to ``https://<redacted>@[::1]/secret``.  Pass 2's bracketed-IPv6 branch
    had no optional userinfo prefix, so it could NOT match the rewritten
    authority — the path ``/secret`` (which may carry credentials) survived
    all three passes.  The fix adds ``(?:[^\\s/@\"']+@)?`` before the host
    alternation so BOTH the raw form AND the pass-1 output are matchable.

    These tests pin the composition gap closure with EXACT redacted forms.
    """

    @pytest.fixture(autouse=True)
    def _import_redact(self):
        """Import redact_credentials once for all tests in this class."""
        scripts_dir = str(
            REPO_ROOT
            / "src"
            / "kiro_crew"
        ...
        self.redact = redact_credentials

    def test_ipv6_userinfo_path_redacted(self):
        """https://user:tok@[::1]/secret/repo.git → path must NOT survive."""
        result = self.redact("https://user:tok_secret123@[::1]/secret/repo.git")
        assert "tok_secret123" not in result, "Token leaked through IPv6-userinfo composition gap"
        assert "/secret" not in result, "Path credential survived IPv6-userinfo composition gap"
        # Exact redacted form: pass 1 redacts userinfo, pass 2 redacts path.
        assert result == "https://<redacted>@[::1]/<redacted>"

    def test_ipv6_userinfo_with_port_path_redacted(self):
        """https://user:tok@[::1]:8443/secret → path must NOT survive."""
        result = self.redact("https://user:tok_secret123@[::1]:8443/secret/repo.git")
        assert "tok_secret123" not in result
        assert "/secret" not in result
        assert result == "https://<redacted>@[::1]:8443/<redacted>"

    def test_ssh_ipv6_userinfo_query_redacted(self):
        """ssh://tok@[2001:db8::7]:22/team?token=x → token and query redacted."""
        result = self.redact("ssh://tok_secret123@[2001:db8::7]:22/team?token=x")
        assert "tok_secret123" not in result, "SSH token leaked"
        assert "token=x" not in result, "Query-string credential survived"
        assert "/team" not in result, "Path survived"
        # Exact form: pass 1 redacts userinfo, pass 2 redacts path+query.
        assert result == "ssh://<redacted>@[2001:db8::7]:22/<redacted>"

    def test_ipv6_full_address_userinfo_path_redacted(self):
        """https://user:tok@[2001:db8::7]/secret → path must NOT survive."""
        result = self.redact("https://user:tok_secret123@[2001:db8::7]/secret/repo.git")
        assert "tok_secret123" not in result
        assert "/secret" not in result
        assert result == "https://<redacted>@[2001:db8::7]/<redacted>"

    def test_ipv6_zone_id_userinfo_path_redacted(self):
        """https://tok@[fe80::1%25eth0]:443/path → path redacted, zone-id preserved."""
        result = self.redact("https://tok_secret123@[fe80::1%25eth0]:443/secret/path")
        assert "tok_secret123" not in result
        assert "/secret" not in result
        assert result == "https://<redacted>@[fe80::1%25eth0]:443/<redacted>"

    def test_raw_userinfo_ipv6_defense_in_depth(self):
        """Raw (un-redacted) userinfo before IPv6 host also works.

        Defense in depth: if pass ordering ever changes, the raw userinfo
        form must also be matchable by pass 2.
        """
        # Simulate a case where pass 1 did NOT run (defense in depth).
        # Pass 2 alone must still handle userinfo@[IPv6]/path.
        from push_guard import _URL_AFTER_AUTHORITY_RE

        text = "https://user:tok@[::1]:9090/secret/path"
        match = _URL_AFTER_AUTHORITY_RE.search(text)
        assert match is not None, "Pass 2 regex must match raw userinfo before IPv6 host"
        # The captured group 1 should include the scheme + userinfo + host + port.
        assert "[::1]:9090" in match.group(1)

    def test_fetch_stderr_ipv6_userinfo_full_integration(self):
        """Fetch stderr with both IPv6 URLs: neither token nor path leak.

        Pins the exact scenario from the PR finding (GPT round 12):
        fetch stderr containing credentialed bracketed-IPv6 origins.
        """
        stderr = (
            "fatal: unable to access 'https://user:tok_secret123@[::1]/secret/repo.git': "
            "Could not resolve host\n"
            "fatal: Could not read from remote 'ssh://tok_secret123@[2001:db8::7]:22/team?token=x'"
        )
        result = self.redact(stderr)
        # Neither credential nor path/query must survive.
        assert "tok_secret123" not in result
        assert "/secret" not in result
        assert "token=x" not in result
        assert "/team" not in result
        # Hosts must survive for diagnostic value.
        assert "[::1]" in result
        assert "[2001:db8::7]" in result


class TestRedactCredentialsIdempotence:
    """Idempotence: applying redact_credentials twice must equal once.

    A redaction function that is not idempotent can mangle its own output
    on a second pass — e.g. ``<redacted>`` could be mistaken for a userinfo
    token and double-redacted.  The three-pass architecture must be stable
    ...
    @pytest.mark.parametrize(
        "input_text",
        [
            # Scheme-bearing userinfo + path
            "https://user:s3cr3t@github.com/org/repo.git",
            # PAT-style token
            "https://ghp_abc123XYZ@github.com/org/repo.git",
            # Query-string credential
            "https://git.example.com/repo?private_token=secret123",
            # Path-embedded token
            "https://git.example.com/tok_secret123/Repo.git",
            # IPv6 with userinfo + path (the round-12 composition gap)
            "https://user:tok_secret123@[::1]/secret/repo.git",
            "ssh://tok_secret123@[2001:db8::7]:22/team?token=x",
            "https://tok@[fe80::1%25eth0]:443/secret",
            # SCP-style remotes
            "tok_secret123@git.example.com:team/Repo.git",
            "tok_secret123@[::1]:team/repo.git",
            "tok_secret123@2001:db8::7: Permission denied",
            # Mixed text with multiple URLs
            (
                "fatal: unable to access 'https://user:tok@[::1]:8443/secret': "
                "error from tok_secret@forge:team/repo.git"
            ),
            # Already-redacted output (must be stable)
            "https://<redacted>@[::1]/<redacted>",
            "https://<redacted>@github.com/<redacted>",
            "<redacted>@git.example.com:team/repo.git",
        ],
        ids=lambda v: v[:60],
    )
    def test_idempotence(self, input_text):
        """redact_credentials(redact_credentials(x)) == redact_credentials(x)."""
        once = self.redact(input_text)
        twice = self.redact(once)
        assert twice == once, (
            f"Idempotence violation:\n"
