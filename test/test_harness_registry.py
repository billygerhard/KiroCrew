...
def _fresh_registry(harnesses: dict | None = None, default_harness: str = "") -> HarnessRegistry:
    """A registry reading a config that declares *harnesses*."""
    agent: dict = {}
...
def test_a_new_operator_harness_appears_after_the_config_changes(tmp_path):
    ...
    _write_config({"agent": {"harnesses": {"later": _operator_descriptor(exe)}}})
    reg.reload()
...
def test_the_file_holding_harness_definitions_is_write_protected_from_agent_tools():
    ...
    ``agent.harnesses`` lives in ``config.json``, which is on the write-only
    ...
    assert security.is_sensitive_write_path(str(config_path()))
    assert security.is_sensitive_write_path(str(config_local_path()))
    # Pinned against the LIST, not just the predicate: removing config.json from
    # the write-protected tier must fail here rather than silently open the key.
    tails = {entry.rsplit("/", 1)[-1] for entry in security.write_protected_home_paths()}
    assert "config.json" in tails
...
# ── Write guard: the config setter ──
#
# `kirocrew config set` is the third reachable write path and the one the other
# two do not cover: it is an ordinary command an agent runs through its bash tool,
# it takes the config lock itself so it never passes the file-edit gate, and
# `--local` writes the overlay the registry also reads. Every test below asserts
# BOTH halves — the command exits non-zero AND nothing landed on disk — so
# deleting the guard fails here instead of quietly persisting the descriptor.

_EXPECTED_REFUSAL = (
    "❌ Refusing this write — agent.harnesses is operator-only: a harness "
    "definition names an executable the gateway spawns, and nothing downstream "
    "clamps that value."
)
...
def _config_files_text() -> str:
    """Both config files' contents concatenated, for "nothing persisted" checks."""
    from kiro_crew.config.loader import config_local_path, config_path
...
@pytest.mark.parametrize(
    "key, value",
    [
        # The key itself, as the reviewer's verified command spells it.
        ("agent.harnesses", '{"evil": {"executable": "/tmp/pwn", "argv": ["{executable}"]}}'),
        # A path INSIDE it: setting one field authors the same executable.
        ("agent.harnesses.evil.executable", "/tmp/pwn"),
        # An ancestor whose new value CARRIES the key — the whole-section form.
        ("agent", '{"harnesses": {"evil": {"executable": "/tmp/pwn"}}}'),
    ],
)
@pytest.mark.parametrize("local", [False, True])
def test_config_set_refuses_to_author_a_harness_definition(key, value, local, capsys):
    """R2.4 on the CLI setter path, base file and overlay alike.

    ``--local`` is parametrized rather than tested separately because the overlay
    takes PRECEDENCE over the base file: a guard on one half only would leave the
    other half as the way around it.

    The MESSAGE is asserted, not just the exit code: the base path also rejects an
    unknown key with status 1, so a bare exit-code assertion would pass for the
    wrong reason on the nested-key case and stop pinning this guard at all.
    """
    from kiro_crew.cli_config import _config_cmd

    with pytest.raises(SystemExit) as excinfo:
        _config_cmd(_set_args(key, value, local=local))
    assert excinfo.value.code == 1
    assert _EXPECTED_REFUSAL in capsys.readouterr().err
    assert "harnesses" not in _config_files_text()


def test_the_refusal_points_at_the_operator_write_path(capsys):
    """A refusal an operator can also trigger has to say where to make the change."""
    from kiro_crew.cli_config import _config_cmd

    with pytest.raises(SystemExit):
        _config_cmd(_set_args("agent.harnesses", "{}"))
    assert "kirocrew config edit" in capsys.readouterr().err


def test_config_set_from_a_file_cannot_smuggle_a_harness_definition(tmp_path, capsys):
    """``--file`` replaces the whole config object, so it authors any key in it."""
    from kiro_crew.cli_config import _config_cmd

    payload = tmp_path / "cfg.json"
    payload.write_text(
        json.dumps({"agent": {"harnesses": {"evil": {"executable": "/tmp/pwn"}}}}),
        encoding="utf-8",
    )
    args = argparse.Namespace(
        config_action="set", key=None, value=None, local=False, file=str(payload)
    )
    with pytest.raises(SystemExit) as excinfo:
        _config_cmd(args)
    assert excinfo.value.code == 1
...
@pytest.mark.parametrize("local", [False, True])
def test_a_benign_key_still_sets(local):
    """The control: the guard is key-scoped, not a general refusal to write config.

    Without this, a guard that refused everything would pass every test above
    while breaking the setter.
    """
    from kiro_crew.cli_config import _config_cmd

    _config_cmd(_set_args("dashboard.bot_name", "Crew", local=local))
    assert "Crew" in _config_files_text()


def test_an_ancestor_write_that_authors_no_harness_still_sets():
    """The ancestor form is decided by the VALUE, not by the key alone.

    ``config set agent '{...}'`` is refused only when the object it writes carries
    a harness definition; an agent section without one is an ordinary write.
    """
    from kiro_crew.cli_config import _config_cmd

    _config_cmd(_set_args("agent", '{"approval_mode": "interactive"}'))
    assert "interactive" in _config_files_text()
    assert "harnesses" not in _config_files_text()


_SEEDED_HARNESS = {"mine": {"executable": "/opt/agy/agy-acp", "argv": ["{executable}", "acp"]}}


def _local_config(payload: dict) -> None:
    """Write ``config.local.json`` under this test's pinned data home."""
    from kiro_crew.config.loader import config_dir, config_local_path

    config_dir().mkdir(parents=True, exist_ok=True)
    config_local_path().write_text(json.dumps(payload), encoding="utf-8")


def test_config_set_cannot_delete_a_harness_by_replacing_its_section(capsys):
    """A section write DELETES the definitions it does not mention.

    ``config set agent '{"approval_mode": "auto"}'`` names nothing protected, so a
    guard reading only the incoming value passes it — and the setter then replaces
    the whole agent node, taking an operator's descriptor with it. Retiring a
    harness a session is expected to run on is a modification of an operator-only
    value, and an agent that can delete an id can re-author it afterwards.
    """
    from kiro_crew.cli_config import _config_cmd

    _write_config({"agent": {"model": "auto", "harnesses": _SEEDED_HARNESS}})

    with pytest.raises(SystemExit) as excinfo:
        _config_cmd(_set_args("agent", '{"approval_mode": "auto"}'))
    assert excinfo.value.code == 1
    assert "agent.harnesses is operator-only" in capsys.readouterr().err
    # The descriptor survives, in the file and in what the registry serves.
    assert "mine" in _config_files_text()
    assert HarnessRegistry().get("mine").executable == "/opt/agy/agy-acp"


def test_config_set_file_cannot_delete_a_harness_by_replacing_the_config(tmp_path, capsys):
    """``--file`` replaces the whole object, so it deletes every key it omits."""
    from kiro_crew.cli_config import _config_cmd

    _write_config({"agent": {"harnesses": _SEEDED_HARNESS}})
    payload = tmp_path / "cfg.json"
    payload.write_text(json.dumps({"agent": {"model": "auto"}}), encoding="utf-8")
    args = argparse.Namespace(
        config_action="set", key=None, value=None, local=False, file=str(payload)
    )
...
def test_config_set_local_cannot_delete_a_harness_the_overlay_defines(capsys):
    """The overlay half of the delete guard.

    The overlay is read by the registry and takes precedence, so a guard on the
    base file alone would leave ``--local`` as the way to wipe a definition.
    """
    from kiro_crew.cli_config import _config_cmd

    _local_config({"agent": {"harnesses": _SEEDED_HARNESS}})

    with pytest.raises(SystemExit) as excinfo:
        _config_cmd(_set_args("agent", '{"approval_mode": "auto"}', local=True))
    assert excinfo.value.code == 1
    assert "agent.harnesses is operator-only" in capsys.readouterr().err
    assert "mine" in _config_files_text()


def test_config_set_local_is_judged_against_the_overlay_not_the_merged_config():
    """The guard reads the file the write LANDS IN, not the effective config.

    Base and overlay merge per leaf, so an overlay section write cannot destroy a
    base-defined harness. Judging this write against the merged config would refuse
    an ordinary ``--local`` section write that deletes nothing.
    """
    from kiro_crew.cli_config import _config_cmd

    _write_config({"agent": {"harnesses": _SEEDED_HARNESS}})

    _config_cmd(_set_args("agent", '{"approval_mode": "interactive"}', local=True))
    assert "interactive" in _config_files_text()
    # And the base definition the merge still serves is untouched.
    assert HarnessRegistry().get("mine").executable == "/opt/agy/agy-acp"


def test_the_registry_exposes_no_writer_for_harness_definitions():
    """The registry reads configuration and never writes it.

    Pinned as an explicit public surface so adding a mutator is a deliberate edit
    to this list rather than an accident that hands the agent a write path.
    """
    public = {
        name
        for name, value in inspect.getmembers(HarnessRegistry, callable)
...
def test_the_config_keys_round_trip(tmp_path):
    import dataclasses
    ...
    exe = _executable(tmp_path / "tool")
    _write_config(
        {"agent": {"default_harness": "mine", "harnesses": {"mine": _operator_descriptor(exe)}}}
    )
    agent = KiroCrewConfig.load().agent
    assert agent.default_harness == "mine"
    assert agent.harnesses["mine"]["executable"] == exe
    # The descriptor survives the dataclass round-trip save() performs.
    assert dataclasses.asdict(agent)["harnesses"]["mine"]["executable"] == exe
...
def test_a_non_object_harness_entry_is_served_as_an_invalid_row(caplog):
    ...
    The loader passes it through un-coerced so the registry's reason channel —
    the only surface an operator sees — can say ``descriptor must be an object``
    under the entry's own id. Dropping it at load would leave Settings with no
    row and no reason for a harness the operator plainly configured.
    """
    from kiro_crew.config.loader import KiroCrewConfig

    _write_config({"agent": {"harnesses": {"mine": "kiro-cli acp"}}})
    assert KiroCrewConfig.load().agent.harnesses == {"mine": "kiro-cli acp"}
...
def test_a_non_object_harnesses_section_reads_as_empty():
    from kiro_crew.config.loader import KiroCrewConfig

    _write_config({"agent": {"harnesses": ["mine"]}})
    assert KiroCrewConfig.load().agent.harnesses == {}
