...
class TestSoloGateRosterCheckBackend:
    ...
    @staticmethod
    def _state(pinned: str | None) -> SimpleNamespace:
        sessions = MagicMock()
    ...
    def _diff(self, state: SimpleNamespace, backend: str) -> str:
        ...
        with patch(
            ...
            return solo_spawn_difference(state, "dashboard:chat-1", backend=backend)
    ...
    def test_unpinned_slot_compares_against_the_deployment_default(self):
        cfg = SimpleNamespace(agent=SimpleNamespace(acp_backend="kas"))
        with patch("kiro_crew.config.loader.KiroCrewConfig.load", return_value=cfg):
            assert self._diff(self._state(""), "kas") == ""
            assert self._diff(self._state(""), "claude") == "backend"
    ...
    def test_unknown_parent_fails_open_and_says_so(self):
        ...
        # Unpinned slot on the kiro default: "" is ambiguous with unknown, so open.
        cfg = SimpleNamespace(agent=SimpleNamespace(acp_backend=""))
        with patch("kiro_crew.config.loader.KiroCrewConfig.load", return_value=cfg):
            assert self._diff(self._state(""), "kas") == "backend (parent unknown)"
