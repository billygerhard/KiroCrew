...
class TestSoloGateRosterCheckBackend:
    ...
    def _diff(self, state: SimpleNamespace, backend: str | None) -> str:
    ...
    def test_unpinned_slot_compares_against_the_deployment_default(self):
        ...
        with patch("kiro_crew.config.loader.KiroCrewConfig.load", return_value=cfg):
            assert self._diff(self._state(None), "kas") == ""
            assert self._diff(self._state(None), "claude") == "backend"
    ...
    def test_kiro_parent_is_comparable(self):
        ...
        with patch("kiro_crew.config.loader.KiroCrewConfig.load", return_value=cfg):
            assert self._diff(self._state(None), "") == ""
            assert self._diff(self._state(None), "kas") == "backend"
