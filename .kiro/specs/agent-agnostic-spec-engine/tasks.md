...
- [x] 12. UI driver (absorb Spec Builder)
  ...
  - [x] 12.2 Review queue surface and gate actions
  ...
  - [x] 12.3 Configuration surface, cost display, and kill switch
...
- [x] 19. Doctor
  ...
  - [x] 19.2 Doctor surfaces and surface equivalence
...
- [x] 20. Engine composition and wiring
  ...
  - [x] 20.2 Entry-point gating and session seeding
  ...
  - [x] 20.5 Resume authority
    ...
    - Pass `StateFindingsSink(state)` when a run-path graph is first built, and pin it. Correcting an earlier note that said task 20.2 had forgotten this: it had not. `build_engine` REQUIRES `findings_sink` with no memory default, and `EngineGraph`'s invariant refuses a `RecordingFindingsSink` by type, so no graph can silently carry the in-memory sink. The real position is narrower and different: `StateFindingsSink` is constructed only in its own tests, because no production graph runs analyses yet — the MCP surface deliberately passes `RefusingFindingsSink`, which is correct for a surface that touches no run. The residual risk is that whoever builds the first run-path graph copies that MCP spelling and gets a graph that passes the invariant while recording nothing, loudly refusing instead of persisting. So this is a construction obligation, not a repair: pass the durable sink, and add the test asserting a run-path graph's sink survives a reopened store — task 12.2 proved the sink itself durable that way but could not assert it of a graph, because none existed
    - Audit the announcer failure class the swallow hides, and check a replaced graph for project coherence. Two minors from 20.2's review, both in files 20.5 already touches. (a) `RunMachine.transition` swallows announcer exceptions so a failed notice cannot unwind a durable transition -- correct -- and the EXPECTED delivery failures are audited, because `HostNotifier.send` wraps them into `NotificationUndelivered` and the seeder records `AWAITING_REVIEW_NOTIFY_FAILED_EVENT`. But an announcer-INTERNAL failure outside that class (notifier construction, or the audit append itself raising) reaches the machine's catch and produces only a `logger.warning`, so a person reading the run's audit log sees a park with no notice and no failure record. Requirement 6.3 wants the gate announced; one audit append in that catch closes it. (b) The `EngineGraph` invariant does not check project coherence, and the non-vacuity control uses `replace(graph, project="other")` -- which is not a no-op: it yields a graph whose project disagrees with the one baked into its machine, seeder, registry and notifier, so `prerequisite_refusal` would gate under one project while the machine audits under another. Either check coherence in `__post_init__` or make the control a genuine no-op and pin the incoherent case separately
    - Decide, explicitly, whether `spec_engine` belongs in `BUILTIN_NAMES`. Verified by 19.2's review: it is absent from `apps/builtins/__init__.py`, and the implementer's inference that this disables the app was mostly WRONG -- the `on_startup` hook runs via the manifest through `discover_builtin_apps()`'s directory walk and `lifecycle.py`, and the MCP server launches from the manifest's own `python3 -m ...engine_mcp.server` command, so neither depends on that list. What the absence actually costs is the dashboard's builtin `register_routes` loop (moot: this app is forbidden from shipping aiohttp routes by its own posture test) and the unused `kirocrew mcp-spec_engine` CLI alias. The app-store metadata half is now closed separately by an `APP_MANIFEST_KEY` entry. So this is a one-line shipping decision with a real but narrow blast radius, and the reviewer's point stands: make it deliberately rather than leaving it silent. One residual the review could not settle without a live gateway: whether the app is enabled by default on a real host
    - _Requirements: 25.4, 9.3, 9.4, 18.5, 6.3_
  - [x] 20.6 Semantic turn provider
...
### Superseded by `spec-engine-own-surface` (recorded 2026-08-17)
...
- **The phrase to stop repeating.** This spec's design overview says the app packages "the absorbed Spec Builder UI". Absorbing another team's app was never this project's to do, and that sentence is the origin of the defect the later spec exists to correct. Read it as a historical statement of intent, not as a plan.
- **Still open here, untouched by that spec.** The delivery-isolation obligation above (delivery stages do not execute in the run's isolated checkout) is explicitly out of scope for `spec-engine-own-surface` and remains this document's open obligation. Requirement 12.6's second clause and Requirement 6.4's missing trace likewise remain open here.
- **One observation for the Prior_App's owner, found while reverting.** No reverted hunk fixed a genuine defect of `spec_builder` at the merge-base. One factual error was noticed and deliberately NOT fixed, because byte-identity forbids it: its `_seed_prompt` docstring claims a builtin's declared skills "are NOT on the agent's skill path", but `bridges.reconcile_app_skills` links manifest-declared skills for enabled non-self-managed apps. That belongs in a report to its owner, not in an edit.
