...
- [ ] 13. App packaging and providers
  - [ ] 13.1 Manifest, discovery skill, and registration
    ...
    - Pass a cancel cascade to `dispatch_tick`. Task 8.4 made it a REQUIRED keyword rather than one defaulting to `None`, deliberately: the tick can build its own spend gate from the stores it holds, but cancelling a run and archiving its spec needs an audit log it does not have, so a default could only have meant skip -- and skipping leaves an item withdrawn mid-run spending until it finishes work nobody wants. The engine's implementation is `review_queue.ReviewQueue`, so the production caller constructs `ReviewQueue(RunMachine(state, config, audit=audit))` and passes it as `cascade=`. Nothing outside tests does yet, which is why it is named here
