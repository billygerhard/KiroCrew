...
- **prerequisites** — a read-only, zero-token preflight per project: do the programs the configured commands need resolve on PATH, do configured provider transports reach, does the base branch exist, is the protected set valid, does the notification channel resolve, does each enabled autonomy level above authoring have a budget ceiling. Results gate rather than decorate: a level whose prerequisites are unmet does not start, and a watch source whose prerequisites are unmet stops dispatching and reports the specific gap instead of dispatching runs that cannot finish. Every blocked run or dispatch is audited.
...
| Prerequisites gate rather than warn | An unattended run that dies on a missing program halfway through has already spent credits and left a half-built branch; a read-only preflight turns that into a refusal with a named fix |
