...
- **content trust** — submitter class is derived **per authored element**, not per item. The item body, each comment on it, and each comment on the review artifact carry their own author and their own class, and a class is never inherited from its container. This closes two escalation paths that a per-item model leaves open: an external party commenting on a maintainer's issue would otherwise ride at maintainer trust, and reviewer comments on a delivery artifact would otherwise drive fix-task dispatch with no author check at all — an unauthenticated instruction channel into an unattended run that also spends the run's budget. Comments from a class not permitted to drive dispatch are quarantined for human release rather than ignored, reusing the screening-release path. Because external text can be edited after it was classified, a changed element re-derives its class and re-applies every decision gated on it before the new content is used; the class, the author, and the content revision relied upon are all recorded, so a trust decision is reconstructable after the fact.
- **tracker writeback** — the return leg of the watcher. Boards differ in commands but agree on concepts (comment, label, state, assign, link), so writeback is not a new mechanism: named lifecycle events map to configured commands under the *same* execution rules as delivery stages, with bundled presets for the public hosts and per-event overrides for an organization's tracker. Three properties earn their keep. It is **at-most-once per run per event** through the ledger, because a watcher re-polls and a resumed run replays — the naive version comments five times on one issue. Its failure is **advisory**: a label that would not set says nothing about whether the spec is good, so it is recorded and surfaced but never fails the run. And its content comes only from **declared templates plus engine values** — never model-composed prose, never the item body echoed back — because the item body is attacker-controlled on a public tracker, and a writeback that quotes it turns the app into an amplifier for whatever was planted there.
...
- **tasks.md dependency graph** — the fenced `json` block under the task dependency graph heading. Canonical form is the wave list: `{"waves": [{"id": 0, "tasks": ["1.1", "1.2"]}, {"id": 1, "tasks": ["2.1"]}]}`, wave ids sequential integers from zero, every incomplete leaf task scheduled in exactly one wave. A graph may additionally carry `dependencies`, an optional object mapping a task number to the task numbers it needs (`{"2.1": ["1.2"]}`); every task named must be scheduled, a dependency must sit in an earlier wave than its dependent, and the declared edges must be acyclic. The edges are what make the acyclicity check meaningful — a wave list on its own cannot express a cycle — so a graph declaring none is acyclic by construction and reports nothing.
...
| Trust is per-author, never per-item | Containers aggregate multiple authors; inheriting a container's class hands every commenter the trust of whoever opened it, which on a public tracker is a one-comment privilege escalation |
| Editable content re-derives its class before reuse | A trust decision taken at claim time describes the content as it was then; acting later on the same element without re-deriving means acting on text nobody authorized |
| Writeback is template-bounded, never model-composed | The watched item is untrusted input; if a model may write free text back to a shared board, an injected instruction gains a publication channel and the app becomes the amplifier. Templates plus engine values keep the write surface finite |
| Writeback failure never fails the run | Board housekeeping and spec correctness are independent; conflating them would discard good work over a tracker permission error |
