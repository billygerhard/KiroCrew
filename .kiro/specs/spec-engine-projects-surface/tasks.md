...
- [ ] 1. Shell landing and navigation
  - [ ] 1.1 First-run nav ordering and the retained-data guard
    - Derive `firstRun` in `SpecEnginePage.tsx` as `!config.isError && config.data?.configured === false` — one derivation consumed by BOTH the landing rule and the nav order, so the two cannot disagree
