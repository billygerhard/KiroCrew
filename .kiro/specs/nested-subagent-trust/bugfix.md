...
a fresh conversation at the chat whose tab held its card.
...
names and a one-clause why, ahead of the tool's own purpose, so the global-feed entry says why it
has no chat while still saying what the tool is for.
...
nothing -- and SHALL treat the conversation as
contested when that root cannot be read (no record, or a nested record
written before the stamp existed; a depth-one pre-stamp record's chat parent
IS the root admission would have stamped and SHALL be used) or names a
different root; a contest established
...
2.12 WHILE a prompt of a run whose card is shown in a chat's tab is parked on
the global approvals feed because the run's trust root is a contested marker
THEN that tab SHALL be told where the prompt went (the run's chip row names
the notification bell) for exactly the duration of the wait, so the run does
not read as waiting for nothing.
