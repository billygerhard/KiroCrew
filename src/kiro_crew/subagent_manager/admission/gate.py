        ...
        *,
        ...
        Args:
            ...
            acp_backend (str): Per-spawn ACP backend override (e.g. an
                edition-registered harness id). ``""`` (the default) is no
                override: the subagent runs on the configured default backend,
                not the parent chat's per-chat pick. A non-empty value must be in
        ...
        # An empty value is no override (the configured default backend, not
        # the parent chat's per-chat pick) and needs no check. A
