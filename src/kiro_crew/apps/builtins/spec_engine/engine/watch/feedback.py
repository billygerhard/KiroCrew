...
**Zero model credits.** These are configured argv lists run through the delivery
executor. Nothing here composes text with a model, and the commands carry engine
values through the run context rather than item text, so no field of a public
tracker item reaches a command position.
"""
...
def load_feedback(config: ConfigStore, source: str) -> dict[str, tuple[CommandTemplate, ...]]:
    ...
    cannot be parsed. An unreadable feedback map must not resolve to "no
    feedback": that would turn a typo into silence on every item, which looks
    exactly like a source nobody configured feedback for.
    """
