...
class GatewayOrchestrator:
    ...
    def _contested_prompt_card(self, request_id: str, parent_session_key: str) -> tuple[str, str]:
        """The tab showing the CARD of a run whose prompt is contested, and the run's id.

        A contested run's prompt is unowned (:meth:`_spawn_approval_slot` names
        no tab, because the tab's Trust must not answer it), but the run's card
        still lives in the tab of the chat that continued it. Someone watching
        that tab sees the run wait with no prompt, so the tab is told where the
        prompt went (``subagent_awaiting_feed``). Resolved from the run the
        prompt belongs to: a spawn prompt names its run in the request id; a
        tool prompt carries the contested marker, whose conversation key names
        the founder, and the one live run of that conversation is the asker.
        ``("", "")`` when nothing resolves, when the run has no tab (its routing
        root is itself a marker or a ``subagent:`` key), or when the prompt is
        not contested at all (an owned prompt already reaches its tab).
        """
        if not is_contested_root(parent_session_key):
            return "", ""
        mgr = getattr(self, "subagent_mgr", None)
        if mgr is None:
            return "", ""
        if request_id.startswith("spawn:"):
            info = mgr.get(request_id.removeprefix("spawn:"))
        else:
            conversation_key = contested_conversation_key(parent_session_key)
            live = [r for r in mgr._conversation_records(conversation_key) if not r.done]
            info = live[0] if len(live) == 1 else None
        if info is None:
            return "", ""
        root = mgr.root_session_key_for(info)
        if not root or is_contested_root(root) or root.startswith("subagent:"):
            return "", ""
        return subagent_event_slot(root), info.id
        ...
        *,
        ...
        async def _approve(event: LLMEvent, parent_session_key: str = "") -> bool:
            ...
            if self.dashboard_state:
                ...
                # A contested run's prompt is unowned and reaches only the global
                # feed, but its card sits in the tab of the chat that continued
                # it, where the run now appears to wait for nothing. That tab is
                # told where the prompt went for exactly as long as it is parked;
                # the row's hint clears when the prompt is answered either way.
                card_slot, card_run_id = (
                    self._contested_prompt_card(request_id, parent_session_key)
                    if not approval_slot and is_contested_root(parent_session_key)
                    else ("", "")
                )
                if card_slot and card_run_id:
                    self.dashboard_state.broadcast_ws(
                        "subagent_awaiting_feed",
                        {"slot": card_slot, "id": card_run_id, "awaiting": True},
                    )
                try:
                    ...
                    if card_slot and card_run_id:
                        self.dashboard_state.broadcast_ws(
                            "subagent_awaiting_feed",
                            {"slot": card_slot, "id": card_run_id, "awaiting": False},
                        )
            if _child_lf:
