...
def parent_slot_backend(state: Any, parent_session: str) -> str | None:
    ...
    ``None`` is reserved for "could not determine", so the caller can compare a
    kiro parent (``""``) against a requested backend instead of failing open on
    it; it still fails OPEN on ``None``, exactly as :func:`parent_slot_model`
