...
should get. The five CONFIG-GRANTABLE capabilities are the exception by design:
a ``capabilities`` claim in the descriptor is recorded at registration
(``register_known_backend(capabilities=...)``) and read by the session path
through the derived ``<capability>_backends()`` accessors in
``agent_sdk.backends``, so a claimed capability answers True at every gate a
builtin member would, and an unclaimed one keeps the default posture.
