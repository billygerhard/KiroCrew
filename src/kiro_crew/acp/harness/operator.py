...
class DescriptorHarness(MembershipHarness):
    ...
    async def resolve_spawn(self, ctx: SpawnContext) -> SpawnPlan:
        ...
        # to the path. This is the last read before exec, so it is the one that
        # catches a binary replaced under an unchanged name: a mismatch refuses
        # this spawn and withdraws selectability until the operator verifies the
        # replacement (routing_verification.spawn_attestation_problem).
        from kiro_crew.acp.harness.operator_registry import revoke_routing_verification
        from kiro_crew.acp.harness.routing_verification import spawn_attestation_problem

        problem = await asyncio.to_thread(spawn_attestation_problem, self._descriptor, exe)
        if problem is not None:
            try:
                revoke_routing_verification(self._descriptor.id, problem)
            except ValueError:
