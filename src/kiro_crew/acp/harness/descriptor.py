...
from kiro_crew.agent_sdk.backends import CONFIG_GRANTABLE_CAPABILITIES
...
#
# The names are IMPORTED from the vocabulary home (``agent_sdk.backends``), where
# registration records a claim and the ``<capability>_backends()`` accessors read
# it: one spelling shared by the validator, the registrar and the readers, so a
# claim that validates here is a claim that takes effect there.

#: Every capability name a descriptor may mention, in declaration order. The
#: vocabulary's own tuple, re-exported under the name this module's callers use.
CAPABILITY_NAMES: tuple[str, ...] = CONFIG_GRANTABLE_CAPABILITIES
    ...
    for index, token in enumerate(tokens):
        ...
        for match in _BRACED_RE.finditer(token):
            ...
            if placeholder not in ARGV_PLACEHOLDERS:
                ...
                reasons.append(
                    f"{where} uses unknown placeholder {placeholder} (allowed: {allowed})"
                )
