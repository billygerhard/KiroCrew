...
from kiro_crew.acp.harness.routing_verification import UNVERIFIED_REASON, is_attested
...
def mark_routing_verified(backend_id: str) -> None:
    """Make an unverified operator backend selectable NOW, after a recorded attestation.

    The boot-time gate reads the attestation file; this is the live half so a
    successful Verify in Settings does not need a gateway restart. It refuses an
    id that is not a registered operator descriptor, and it re-reads the store
    rather than trusting the caller: the attestation on disk is the fact, this
    function only projects it into the selectable registry.
    """
    ...
    if not is_attested(descriptor):
        raise ValueError(
            f"no routing attestation matches the current descriptor for {backend_id!r}"
        )
    register_selectable_backend(backend_id)
