...
- **Operator descriptors**, parsed from ``agent.harnesses``: pure data, no
...
- the **file-edit tool**: ``config.json`` is on
...
- the **Settings PATCH** surface (``dashboard.handlers.core._EDITABLE_CONFIG``)
  is an allowlist that does not carry ``agent.harnesses``;
- the **config setter**: ``agent.harnesses`` is on
  ``security._WRITE_PROTECTED_CONFIG_KEYS``, so ``kirocrew config set`` — an
  ordinary command an agent runs through its bash tool, which takes the config
  lock itself and never passes the file-edit gate — refuses the key, its nested
  forms, and an ancestor write that replaces the node holding it, whether that
  write AUTHORS a definition (its value carries the key) or DELETES one (the
  file does), base and ``--local`` alike, plus the whole-object ``--file`` form.

This module exposes no writer at all, and ``test_harness_registry.py`` pins all
three halves.

**One write path is deliberately left open, and it is worth naming precisely.**
Neither of the two raw-shell forms — ``echo '{...}' >
~/.kiro/crew/config.json`` and its ``cd``-relative equivalent — is refused,
because closing them means adding ``config.json`` to
``security._WRITE_PROTECTED_BASH_LEAVES``, whose matcher is verb-INDEPENDENT and
would therefore deny the routine config READS the write-only tier exists to keep
allowed (the dashboard file viewer, ``cat``, knowledge indexing). What the gap
costs is bounded by this registry never trusting on-disk content it cannot
validate: a descriptor that fails validation is excluded from every listing with
a recorded reason, so a garbled or partially-written entry is inert. The residual
is therefore narrower than "an agent can write config.json" but is real — a
SHAPE-VALID malicious descriptor planted by raw redirect is honoured — and it is
the same exposure ``config.json`` carries generally today, accepted here rather
than solved.
"""
...
class HarnessRegistry:
    ...
    def _operator_section(self) -> tuple[dict, str]:
        """``(agent.harnesses, agent.default_harness)`` as stored.
        ...
        serving — one broken file must not cost the default harness.
        """
        try:
            ...
            logger.warning("harness registry: config unreadable; serving bundled harnesses only")
            return {}, ""
        raw = getattr(agent, "harnesses", None)
