...
from typing import Any, Dict, FrozenSet, Iterator, Mapping, Optional, Set, Tuple
...
# ── Config-grantable capability names ──
#
# The five capability claims a config-authored descriptor may make, each naming ONE
# frozen ``ACP_BACKENDS_*`` membership set below that the claim grows. Defined HERE,
# in the vocabulary home, so ``acp.harness.descriptor`` validates against the same
# spelling registration records and the accessors read -- three copies of the list
# is how a typo in one silently becomes an unclaimable capability. Kiro-only sets
# (``internal_sandbox``, ``session_sharing``, ``acp_runtime_pool``,
# ``kiro_identity_store``) are deliberately NOT in this vocabulary: a descriptor
# cannot claim them, so a config-authored host never reaches the kiro-only paths.
...
#: Every config-grantable capability name, in declaration order.
CONFIG_GRANTABLE_CAPABILITIES: Tuple[str, ...] = (
...
#: Registered ids that OPTED IN to each config-grantable capability, keyed by the
#: capability name a descriptor spells (``"session_mcp_array"`` and its siblings --
#: the same five names ``acp.harness.descriptor.CAPABILITY_NAMES`` validates). Empty
#: at rest, so each ``<capability>_backends()`` accessor below equals its frozen
#: ``ACP_BACKENDS_*`` set until a descriptor claims the capability. This is the seam
#: the descriptor's ``capabilities`` block exists for: a capability is an opt-in claim
#: that maps to a membership answer, and membership is decided per harness -- so a
#: claim is RECORDED here at registration and READ through the accessor at every
#: session-path site that asks the question. Unclaimed capabilities carry the
#: frozen set's default posture, exactly as before. Kept as a map of sets rather than
#: five module sets so the reset and the registration have one shape to walk.
_registered_capabilities: Dict[str, Set[str]] = {}
...
def register_known_backend(
    ...
    * the **id** into ``_registered_known``, so ``ACP_BACKENDS_KNOWN`` now contains
      ...
      a claimed capability is recorded beside them, see the last bullet, and every
      unclaimed one carries the default posture);
    ...
    * each **capability** in ``capabilities`` that is ``True`` into
      ``_registered_capabilities[<name>]``, so the matching
      ``<capability>_backends()`` accessor reports the id as a member and every
      site that asks the question through that accessor treats the host the way it
      treats a builtin member. Names must come from
      ``CONFIG_GRANTABLE_CAPABILITIES`` -- an unknown or kiro-only name is refused,
      so a descriptor cannot claim a path this build never meant to grant by
      configuration. ``False``/absent claims record nothing (the default posture).
    ...
    claimed: Tuple[str, ...] = ()
    if capabilities:
        unknown = sorted(name for name in capabilities if name not in CONFIG_GRANTABLE_CAPABILITIES)
        if unknown:
            raise ValueError(
                f"cannot register {backend_id!r}: capabilities {unknown} are not "
                f"config-grantable; allowed: {list(CONFIG_GRANTABLE_CAPABILITIES)}"
            )
        claimed = tuple(name for name in CONFIG_GRANTABLE_CAPABILITIES if capabilities.get(name))
    ...
    for name in claimed:
        _registered_capabilities.setdefault(name, set()).add(backend_id)
...
# ── Derived capability membership: frozen vocabulary + registered claims ──
#
# One accessor per config-grantable capability, each the derived twin of a frozen
# ``ACP_BACKENDS_*`` set below, in exactly the shape ``acp_runtime_backends()`` gives
# the runtime set: the vocabulary set stays a literal (the parity gate reserves the
# ``ACP_BACKENDS_*`` spelling for vocabulary, and a builtin's membership stays a
# visible edit), and the DERIVED answer -- vocabulary plus whatever a descriptor
# claimed at registration -- is a function. Every site that asks "does this backend
# have capability X?" on a session path reads the accessor, so a config-authored
# host that claimed X takes the same branch a builtin member takes. Empty
# registrations make each accessor identical to its frozen set, which is the whole
# default posture: a host that has demonstrated nothing gets nothing.


def _capability_backends(name: str, frozen: FrozenSet[str]) -> FrozenSet[str]:
    registered = _registered_capabilities.get(name)
    if not registered:
        return frozen
    return frozenset(frozen | registered)


def session_mcp_array_backends() -> FrozenSet[str]:
    """``ACP_BACKENDS_SESSION_MCP_ARRAY`` plus registered ids that claimed it."""
    return _capability_backends(CAPABILITY_SESSION_MCP_ARRAY, ACP_BACKENDS_SESSION_MCP_ARRAY)


def harness_owned_sessions_backends() -> FrozenSet[str]:
    """``ACP_BACKENDS_HARNESS_OWNED_SESSIONS`` plus registered ids that claimed it."""
    return _capability_backends(
        CAPABILITY_HARNESS_OWNED_SESSIONS, ACP_BACKENDS_HARNESS_OWNED_SESSIONS
    )


def load_without_modes_backends() -> FrozenSet[str]:
    """``ACP_BACKENDS_LOAD_WITHOUT_MODES`` plus registered ids that claimed it."""
    return _capability_backends(CAPABILITY_LOAD_WITHOUT_MODES, ACP_BACKENDS_LOAD_WITHOUT_MODES)


def model_via_config_option_backends() -> FrozenSet[str]:
    """``ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION`` plus registered ids that claimed it."""
    return _capability_backends(
        CAPABILITY_MODEL_VIA_CONFIG_OPTION, ACP_BACKENDS_MODEL_VIA_CONFIG_OPTION
    )


def advertised_model_selection_backends() -> FrozenSet[str]:
    """``ACP_BACKENDS_ADVERTISED_MODEL_SELECTION`` plus registered ids that claimed it."""
    return _capability_backends(
        CAPABILITY_ADVERTISED_MODEL_SELECTION, ACP_BACKENDS_ADVERTISED_MODEL_SELECTION
    )


#: Accessor per capability name -- the one table registration, the descriptor layer
#: and the tests walk to prove every grantable capability has a live reader.
CAPABILITY_BACKENDS_ACCESSORS = {
    CAPABILITY_SESSION_MCP_ARRAY: session_mcp_array_backends,
    CAPABILITY_HARNESS_OWNED_SESSIONS: harness_owned_sessions_backends,
    CAPABILITY_LOAD_WITHOUT_MODES: load_without_modes_backends,
    CAPABILITY_MODEL_VIA_CONFIG_OPTION: model_via_config_option_backends,
    CAPABILITY_ADVERTISED_MODEL_SELECTION: advertised_model_selection_backends,
}
