...
async def verify_operator_backend_routing(backend_id: str, cfg: Any) -> dict[str, Any]:
    ...
    *cfg* is the loaded ``KiroCrewConfig``; the probe builds the DESCRIPTOR's own
    ...
    configured default agent and sandbox -- rather than calling the per-chat
    ...
    def build_provider(session_key: str, cwd: str) -> Any:
        ...
        return AcpProvider(
            work_dir=cwd,
            agent=(getattr(agent_cfg, "default_agent", "") or None),
            sandbox_mode=getattr(agent_cfg, "sandbox", "auto") or "auto",
            session_key=session_key,
            acp_backend=backend_id,
        )

    result = await verify_routing(descriptor, build_provider)
    payload = result.as_dict()
    payload["selectable"] = False
    if result.verified:
        # The probe resolved and digested the binary before it ran and re-checked
        # it after; the record is bound to THOSE bytes, and refused if the file has
        # changed since. Digesting and rewriting the store are file I/O -- off the
        # loop, like every other blocking call the gateway makes.
        try:
