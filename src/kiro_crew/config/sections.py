...
class AgentConfig(_StoredBackendSpelling):
    ...
    default_harness: str = field(
        ...
        metadata=_meta(
            ...
            "'claude'; an operator harness uses its key from agent.harnesses.",
        ),
    )
    # Annotated ``dict[str, dict]`` because an object per entry is the CONTRACT
    # the generated JSON schema publishes, and jsonschema warns on an entry that
    # violates it. A violating entry is still kept rather than stripped: the only
    # reader is the harness registry, whose validator answers "descriptor must be
    # an object" and files that reason under the entry's own id — the one place an
    # operator can see what is wrong with it.
    harnesses: dict[str, dict] = field(
        default_factory=dict,
        metadata=_meta(
            "Custom ACP Harnesses",
            "Operator-defined ACP harnesses, keyed by harness id. Each value is a "
            "descriptor: executable (absolute path or PATH name), argv (token "
            "list using only {executable}/{agent}/{model}/{workdir}), optional "
            "agent_args/model_args blocks, capabilities (all default off), "
            "model_source ('acp_advertised' or 'static' plus models), and "
            "mcp_delivery ('wire_fed' default, or 'file_fed'). Each harness "
            "authenticates itself — Kiro Crew stores no provider credentials. An "
            "entry that fails validation is excluded from every selection surface "
            "with a diagnosable reason and leaves the others untouched. WRITTEN "
            "BY THE OPERATOR ONLY: a descriptor names a binary Kiro Crew will "
            "spawn, so config.json is write-protected against the agent's own "
            "tools and this key is not on the Settings PATCH allowlist.",
        ),
    )
...
def _read_harnesses(value: object) -> dict[str, Any]:
    """Read ``agent.harnesses`` as a mapping of id to raw descriptor.

    The descriptors themselves are passed through VERBATIM and deliberately not
    validated here — whatever shape they arrive in. The harness registry
    validates each one and records a per-harness reason it can show the
    operator, which is what keeps one bad entry from costing the whole section;
    rejecting them at load time would either drop every harness for one mistake
    or leave the loader inventing reasons it has nowhere to render.

    That includes an entry that is not an object at all: the registry's reason
    channel describes that shape too (``descriptor must be an object``), so
    dropping it here is the one failure an operator could not diagnose — the id
    would be simply absent from Settings, with no row and no reason, which reads
    as "Kiro Crew ignored my harness" rather than as "fix this entry".

    Only the SECTION's own shape is decided here, because a non-mapping section
    carries no ids to file reasons under.
    """
