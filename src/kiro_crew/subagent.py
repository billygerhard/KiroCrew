...
# Leads the run's own words on a tool prompt, so the claim is read as the run's.
CONTESTED_PROMPT_RUN_SAYS = "The run says:"


def contested_spawn_note() -> str:
    """The body of a contested spawn prompt: the state and its why, then the remedy.

    The same shape as :func:`contested_tool_note`, so the feed card's two lines
    say what the prompt is (the state) and why, and the remedy -- whole in the
    Review panel, where the decision is made -- follows.
    """
    return f"{CONTESTED_PROMPT_STATE} ({CONTESTED_PROMPT_WHY}). {CONTESTED_PROMPT_REMEDY}."


def contested_tool_note(purpose: str = "") -> str:
    """The body of a contested tool prompt: state and a one-clause why, the purpose, then the remedy, as paragraphs.
    ...
    a parenthesis. The remedy comes LAST: the feed card shows the body's first
    two lines, which must be the state and what the tool is for, while the
    Review panel shows everything -- so a user who only ever sees tool prompts
    still finds the way out where they decide, without the card filling with
    boilerplate on every prompt the run raises.
    """
    # The three parts are three PARAGRAPHS, and the middle one is ATTRIBUTED:
    ...
    # the claim is led with who is speaking and set as a quote, which the
    # Review panel renders as a distinct block and the feed card's plain
    # two-line excerpt still reads as prose after the state.
    head = f"{CONTESTED_PROMPT_STATE} ({CONTESTED_PROMPT_WHY})."
    body = purpose.strip()
    if body and not body.endswith((".", "!", "?")):
        body += "."
    if body:
        body = f"{CONTESTED_PROMPT_RUN_SAYS}\n\n" + "\n".join(
            f"> {line}" if line else ">" for line in body.splitlines()
        )
    return "\n\n".join(part for part in (head, body, f"{CONTESTED_PROMPT_REMEDY}.") if part)
