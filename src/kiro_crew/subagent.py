...
def contested_conversation_key(root_session_key: str) -> str:
    """The conversation key a :func:`contested_root` marker was made from; ``""`` for any other key."""
    if not is_contested_root(root_session_key):
        return ""
    return root_session_key[len(_CONTESTED_ROOT_PREFIX) :]
...
# never "composer". On the spawn prompt the body leads with the remedy's verb,
# so the actionable words survive the card's two-line truncation (the state
# repeats in the Review panel anyway), then the state and the why; on a tool
# prompt the state with the why as a parenthetical clause and then the tool's
# own purpose, since repeating the remedy on every prompt the run raises would
# fill the card with boilerplate and hide what the tool is for.
CONTESTED_PROMPT_STATE = "This run was continued from more than one chat"
CONTESTED_PROMPT_WHY = "no single chat's trust applies"
CONTESTED_PROMPT_REMEDY = "Approve it, or start it again from one chat"


def contested_spawn_note() -> str:
    """The body of a contested spawn prompt: remedy first, then the state and why."""
    return (
        f"{CONTESTED_PROMPT_REMEDY} — {CONTESTED_PROMPT_STATE[0].lower()}"
        f"{CONTESTED_PROMPT_STATE[1:]}, so {CONTESTED_PROMPT_WHY}."
    )


def contested_tool_note(purpose: str = "") -> str:
    """The body of a contested tool prompt: state with a one-clause why, a period, then the purpose.

    A user can meet a tool prompt first (the spawn prompt approved earlier, or
    missed), so the state alone would leave them guessing; the why rides along in
    a parenthesis, and the remedy stays with the spawn prompt so the tool's own
    purpose is not buried under boilerplate on every prompt the run raises.
    """
    # A period, not a dash, between the state and the tool's own purpose: after
    # a dash the purpose reads as a remedy clause of the state sentence.
    head = f"{CONTESTED_PROMPT_STATE} ({CONTESTED_PROMPT_WHY})."
    ...
    def conversation_root_for_new_run(
        ...
        *durable_root*: the value of the founder's durable run record, where
        ...
        if not founding:
            ...
            if not durable:
                return contested_root(conversation_key)  # unknown founder: fail closed
            if is_contested_root(durable):
                return durable
            founding.add(durable)
        # Two independent founders (concurrent continuations admitted after the
        # original's record was evicted) disagree: nobody owns the key.
