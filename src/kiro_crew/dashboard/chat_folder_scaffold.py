...
def preview_payload(tree: CandidateTree, existing_dirs: set[str]) -> dict[str, Any]:
    ...
    return {
        "root": tree.root,
        "root_name": folder_display_name(tree.root),
...
def _scaffold_response(
    ...
    return {
        "root": tree.root,
        # The folder every created child is ultimately nested under, whether this
        # call created it or found it already there. Empty only when it could
        # neither be created nor found.
        "root_folder_id": folder_ids.get(tree.root, ""),
        "created": created,
...
async def _create_selection(
    state: DashboardState, tree: CandidateTree, selected: set[str]
) -> dict[str, Any]:
    ...
    if tree.root in existing:
        ...
        try:
            folder = await create_folder_record(
                state, name=folder_display_name(tree.root), project_dir=tree.root
            )
        except (FolderCreateError, OSError) as exc:
            ...
            return _scaffold_response(tree, folder_ids, created, skipped_existing, failed)
        folder_ids[tree.root] = str(folder["id"])
        created.append({"path": tree.root, "folder_id": str(folder["id"]), "name": folder["name"]})
...
async def api_chat_folders_scaffold(request: web.Request) -> web.Response:
    """POST /api/chat/folders/scaffold — create the confirmed selection.
    ...
    payload = await _create_selection(state, tree, set(selected))
    if payload["created"]:
