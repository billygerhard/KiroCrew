...
async def api_backend_verify(request: web.Request) -> web.Response:
    ...
    asked and nothing was written. See that module for the verdict table.
    """
    ...
    if backend_id not in unverified_operator_harnesses():
        # Not registered, or registered and already selectable, or unroutable:
        # none of these has a pending routing claim this action can settle.
        return web.json_response(
    ...
    async with _verify_lock:
        ...
        try:
            result = await verify_operator_backend_routing(backend_id, factory)
