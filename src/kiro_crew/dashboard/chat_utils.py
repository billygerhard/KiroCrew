...
from kiro_crew.slack.outbound import expire_options, options_edit_lock
...
async def expire_slack_options(
    ...
    if settled:
        # Remove by identity against the CURRENT tuple, not by reassigning a
        # remembered one: a turn that finished while we awaited Slack may have
        # recorded its own control here, and it must survive.
        slot._slack_options_posted = tuple(
            posted for posted in slot._slack_options_posted if posted not in settled
        )
