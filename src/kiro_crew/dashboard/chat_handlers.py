...
async def api_chat_slot_backend(request: web.Request) -> web.Response:
    ...
    async with contextlib.AsyncExitStack() as _stack:
        ...
        slot.acp_backend = backend
        logger.info(
            "Slot %s backend switched to %s, resetting session",
            name,
        ...
        _broadcast_context_reset(state, slot.key, None)
        # Durably persist the pin before reporting success. Every other
        # slot-metadata route (model / tags / folder / pin) commits through
        # save_slot_off_loop(force=True, best_effort=False); the backend switch
        # was only calling push_slots_update() (a sidebar broadcast, not a disk
        # write), so a gateway restart lost the pin and the next turn silently
        # ran on the global backend. force=True carries the empty-window merge
        # for a message-less slot (a backend picked on the welcome screen before
        # the first turn), so a fresh backend-only slot persists too. On a
        # failed write, roll the pin back and surface a coded 500 the caller
        # can retry -- mirroring the model route's mutate-persist-rollback
        # shape.
        #
        ...
        # exactly one request is in the mutate-persist-rollback window, so a
